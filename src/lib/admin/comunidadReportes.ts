...
export type ReporteComunidadPendiente = {
  ...
  contenidoId: string;
  /** Slug del post al que pertenece (el mismo post si `tipoContenido` es "publicación"), para enlazar al hilo. */
  postSlug: string;
  autorNombre: string;
  preview: string;
};
...
export async function getReportesComunidadPendientes(): Promise<ReporteComunidadPendiente[]> {
  ...
  // El enlace al hilo va por slug. Un post reportado ya lo trae; para una
  // respuesta hace falta el de su post, que no siempre está entre los
  // reportados.
  const idsPostPadre = [
  ...
  const resultado = reportes.flatMap((r): ReporteComunidadPendiente[] => {
    ...
    const idPostHilo = esPost ? r.id_post! : respuesta!.id_post;
    ...
    return [
      {
        ...
        // Sin slug (el post no se pudo leer) queda el UUID: la página de
        // detalle lo acepta y redirige.
        postSlug: slugPorPostId.get(idPostHilo) ?? idPostHilo,
